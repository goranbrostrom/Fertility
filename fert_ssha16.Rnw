...
<<lexis, fig.height = 3.3, fig.cap = "The sampling frame (solid lines).">>=
...
x <- head(fert1[, c("id", "lopnr", "region", "hisclass", "period", "cohort", "enter", "exit", "event")])
print(xtable(x, digits = c(0,0, 0,0,0,0, 0,3, 3, 0), caption = "First six records from the data file (selected columns).", label = "tab:sixrow"), include.rownames = FALSE)
@
...
There are 225\,792 unique combinations of the levels of the expanatory variables and the age-period-cohort combinations, but many of them are either structural or random zeros, so the total number of observed combinations is only 6\,228. The original data frame contains 300\,579 records. 
...
cohort. It is governed by the {\tt 'm'} of \citet{ct78}. Similarly, the
...
Table~\ref{tab:sigthree}. Then, in what follows, the results are graphically reported.
...
<<cohorttmfr, fig.cap = "TMFR by hisclass and cohort.">>=
source("R/kohort.R")
...
<<plotmhc1, fig.cap = "Coale-Trussell's m by hisclass and period, successful births.">>=
source("R/getMmhc.R")
y <- matrix(0, ncol = 8, nrow = 6)
##kTab <- kTab[kTab$region == "ske", ]
pp <- levels(kTab$period)
...
<<cohorttmfr1, fig.cap = "TMFR by hisclass and cohort, successful births.">>=
source("R/kohort.R")
y <- matrix(0, nrow = 6, ncol = 5)
koh <- levels(kids$cohort)
for (i in 1:5){
