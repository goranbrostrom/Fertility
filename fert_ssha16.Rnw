...
round(summary(fit)$coef[, 1:2], 4)
@

From this we get that $\hat{M} = \exp(-0.3716) = 0.690$ and $\hat{m} = 0.499$, that is, strong signs of both spacing (small {\tt M}) and stopping (large {\tt m}). Note that the {\tt M = (Intercept)} is evaluated for the reference category, \emph{rural} residence and \emph{marriage age} {\tt (15, 25]}.
...
plot(x, y1[1, -1], col = 1, type = "b", ylab = "M", ylim = c(0, 1.1),
     axes = FALSE, xlab = "Period", xlim = c(1851, 1951), lwd = 2, main = 'M')
axis(1, at = c(1851, x, 1950), las = 2)
axis(2)
axis(4)
box()
for (j in 2:6){
    lines(x, y1[j, -1], col = j, type = "b", pch = j, lty = j, lwd = 2)
}
...
abline(h = 0)
for (j in 2:6){
    lines(x, y2[j, -1], col = j, type = "b", pch = j, lty = j, lwd = 2)
}
...
plot(x, y1[1, ], col = 1, type = "b", ylab = "M", ylim = c(0, 1.3),
     axes = FALSE, xlab = "Cohort", xlim = c(1821, 1921), lwd = 2, main = 'M')
axis(1, at = c(1821, x, 1920), las = 2)
axis(2, las = 1)
axis(4, las = 1)
box()
for (j in 2:6){
    lines(x, y1[j, ], col = j, type = "b", pch = j, lty = j, lwd = 2)
}
...
box()
for (j in 2:6){
    lines(x, y2[j, ], col = j, type = "b", pch = j, lty = j, lwd = 2)
}
...
plot(x, y[1, ], col = 1, type = "b", ylab = "TMFR", ylim = c(0, 10),
     axes = FALSE, xlab = "Period", xlim = c(1821, 1921), lwd = 2)
...
x <- c(1830, 1850, 1870, 1890, 1910)
plot(x, y[1, ], type = "b", ylim = c(0, 5), xlab = "Cohort", ylab = "Births per woman", las = 1)
axis(4, las = 1)
for(i in 2:6){
...
As expected, the overall levels are much lower than the TMFR, but what stands out is the farmers' relatively high level. An explanation to this is that the average fertile period length per married woman is 15 years for farmers, but only 10 years for non-farmers. And that may be explained by the fact that farmers on average are more stationary than others.
...
Then the development by cohort and hisclass, see Figure~\ref{fig:plotmcohhc1}.
...
% <<plotmhcTMFR301, fig.cap = "TMFR at 30 by hisclass and period, successful births.">>=
% ##source("R/getMmhc.R")
% y <- matrix(0, ncol = 8, nrow = 6)
% ##kTab <- kTab[kTab$region == "ske", ]
% pp <- levels(kTab$period)
...
<<cohorttmfr1, fig.cap = "TMFR by hisclass and cohort, surviving births.">>=
source("R/kohort.R")
y <- matrix(0, nrow = 6, ncol = 5)
koh <- levels(kids$cohort)
for (i in 1:5){
...
x <- c(1830, 1850, 1870, 1890, 1910)
plot(x, y[1, ], type = "b", ylim = c(0, 7), xlab = "Cohort", ylab = "TMFR", las = 1)
axis(4, las = 1)
for(i in 2:6){
    lines(x, y[i, ], col = i, lty = i, pch = i, type = "b")
}
