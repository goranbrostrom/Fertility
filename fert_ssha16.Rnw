...
<<setup, include = FALSE>>=
knitr::opts_chunk$set(cache=FALSE,echo=FALSE,message=FALSE,warning=FALSE,comment=
NA)##$
@
...
kTab <- aggregate(kids[, c("event", "exposure")],
...
\subsection{The Coale-Trussell model}

We will assume a piecewise constant hazard model on  the age span 20--50, with jumps every fifth year. We apply the restriction proposed by \citet{ac71} and \citet{ct78}:

\begin{equation}
\lambda(t) = M n(t) \exp\{-m v(t)\}, \quad 20 \le t < 50,
\end{equation}
where 
\begin{equation}
n(t) = \begin{cases}
...
The parameter $m$ governs the form of the (cumulative) hazard function, while $M$ measures level. Figure~\ref{fig:plMm} shows two cases, $(m, M) = (0, 1)$ and $(m, M) = (1, 1.57)$. These two cases generate the same expected number of births over a full reproductive period (20--50). The first is an example of \emph{spacing}, but no \emph{stopping}, while the second is a case of stopping (and eventually spacing).

<<plMm, fig.cap = "The Coale-Trussell distribution, two examples: (m = 0, M = 1) is solid, (m = 1, M = 1.57) is dashed.", fig.height = 4>>=
...
The parameter $M$ measures level, but note that keeping $M$ fixed and increasing $m$ lowers the total hazard up to age 50.
...
data set and followed throghout their fertility period, but not after December 31, 1950. For each woman, the exact timing of her deliveries (if any) after the onset of the first marriage are recorded.

Figure~\ref{fig:lexis} shows the sampling frame.
...
Table~\ref{tab:hisc} shows the distribution over socioeconomic status and time periods.

<<tabh
Tabulating data \citep{gb85}.
...
x <- c(1835, 1865, 1890, 1905, 1915, 1925, 1935, 1945)
plot(x, y[1, ], col = 1, type = "b", ylab = "m", ylim = c(0, 1.3),
     axes = FALSE, xlab = "Period")
axis(1, at = x, las = 2)
axis(2)
box()
for (j in 2:4){
    lines(x, y[j, ], col = j + 2, type = "b", pch = j, lty = j)
}
