...
the demographic transition, northern Sweden 1821--1950\footnote{Paper
  presented at the 41st Annual Meeting of the Social Science History
  Association: Chicago, November 17--20, 2016.}}
\author{Göran Broström\footnote{email: \texttt{goran.brostrom@umu.se}} \\
{\sc Cedar}, Umeå University \\
SE-901\,87 Umeå, Sweden}% \\
%\texttt{goran.brostrom@umu.se}}
%\email{goran.brostrom@umu.se}
...
studied. It is found that the fertility decline starts in the upper classes
in the decades prior to 1900. The farmers are characterized by relatively
high fertility in high ages even in the end of the study period.

A stopping behavior seems to dominate a spacing one, especially along
cohorts. The effect of declining infant mortality over time is minor.
...
and \citet{tbdr14}. One perhaps convincing theory is that the fertility
...
also two thoughtful papers by \citet{ni92,ni11}.  
...
Our data cover the time period 1821--1950, see Figure~\ref{fig:lexis}, and the geographical area defined by the Skellefteå and Umeå towns with surrounding rural parishes.
...
\end{center}
\end{figure}
A detailed map of the parishes is found in Figure~\ref{fig:map}. The Skellefteå region was one big parish in the eighteenth century. It successively split up in smaller parishes, and Figure~\ref{fig:map} shows the status in the year 1915. In our data, \emph{Fällfors} is part of \emph{Byske}, and \emph{Lövånger} and \emph{Burträsk} are not present at all.
% 
\begin{figure}
\begin{center}  
\includegraphics[height=14cm]{myfigs/SKEUME161017.pdf}
\caption{The Skellefte{\aa} and Ume{\aa} regions.}
\label{fig:map}
\end{center}
\end{figure}

The development of population size in the two regions are shown in Figure~\ref{fig:plotpopsize}.
...
<<plotpopsize, fig.height = 5, fig.cap = "Population size by year and region.">>=
...
The population size as defined in our data sets was 19\,000 on January 1,
1900, 33\,000 on 
January 1, 1950 and 104\,000 when the end of the century.
...
\emph{stopping} and \emph{spacing} behavior over time, models for
\emph{birth intervals} have been employed
\citep{bedr06,drsc14,tbdr14,bras14}, but in my opinion this approach is too
indirect. Instead I argue for the \emph{counting process} approach, where
...
\emph{last}, and ends with the dissolution of marriage or age 50, whichever
comes \emph{first}. We also allow intervals to start and end with migration
...
at the \emph{date of marriage}, but it complicates things in some ways, one
...
%<<countbir, echo=FALSE,fig.height=3>>=
<<countbir, echo=FALSE,fig.height=3, fig.cap = "Birth history as a counting process, woman with id 233.">>=
source("R/plotid.R")
...
<<lexis, fig.height = 3.3, fig.cap = "The sampling frame (solid lines) by married woman's age.">>=
...
Central covariates are family {\sc Hisclass}, %{\tt social branch} (with categories {\tt official}, {\tt farm\-ing}, {\tt business} and {\tt worker}), 
{\tt mother's age at marriage}, and mother's {\tt birth\-date}. The rows corresponding to woman No.\ 233 in our data file are (with selected columns) shown in Table~\ref{tab:sixrow}.
...
x <- fert1[fert1$id == 233, c("id", "lopnr", "parish", "hisclass", "birthdate", "enter", "exit", "event", "inf.death")]
...
<<hiscske, fig.cap = "Exposure by hisclass and time period, Skellefte{\\aa} (top) and Ume{\\aa} (bottom). Hisclasses are ordered by number from top (1) to bottom (6).", fig.scap = "Exposure by hisclass and time period.", fig.height = 9>>=
...
zz <- t(yy[-(1:3), ])
barplot(zz[6:1,], col = 6:1, las = 1, main = "Skellefteå", names.arg = c("1901-10", "1911-20", "1921-30", "1931-40", "1941-50"), ylab = "Per cent")
...
yy <- round(prop.table(xx, 1) * 100, 1)
zz <- t(yy[-(1:3), ])
barplot(zz[6:1, ], col = 6:1, las = 1, main = "Umeå", names.arg = c("1901-10", "1911-20", "1921-30", "1931-40", "1941-50"), ylab = "Per cent")
par(oldpar)
...
\item[parish] one of {\tt Umeå (town)}, {\tt Umeå lands (rural}, {\tt
    Skellefteå (town)}, {\tt Skellefteå lands (rural)}, {\tt Byske}, {\tt Bureå}, {\tt Norsjö}, {\tt Jörn}, and {\tt Malå}.
...
\item[event1] Indicator of a birth at {\tt exit} that survives age one.
...
\item[N] Constants from the Coale-Trussell model.
\item[V] Constants from the Coale-Trussell model.
\end{description}
...
xtable(head(kTab[, names(kTab)[2:8]]), caption = "First rows in compact multiway table. Columns representing explanatory variables.", 
       label = "tab:compact")
@
...
In Figure~\ref{fig:aggre} is shown how the \emph{age-period-cohort} plane is split up for each combination of the levels of the explanatory variables.

<<aggre, fig.height = 3.5, fig.cap = "Lexis split. In each sub-area the number of births and total exposure time are calculated.", fig.scap = "Lexis split.">>=
plot(c(1851, 1951), c(50, 50), type = "l", xlim = c(1821, 1951), ylim = c(20, 50), col = "blue", 
...
combinations is only 6\,228. The original data frame contains 300\,579
records.  
...
and given the original data, the tables we create form \emph{sufficient
  statistics}. This means that, given our tables, the extra information in
the original data is pure noise. It will \emph{not} improve our results.  

That is the good news. The back side is that the "pure noise" can be used
to judge model fit. If an analysis shows that it is \emph{not} pure noise,
...
each of our cells as following a \emph{Poisson} distribution and write down
the resulting likelihood function. But note carefully that this does
\emph{not} imply that the numbers of births really follow a Poisson
...
These ideas are not new, see e.g.\  \citet{gb85}. Long before software for
...
loglinear models for contingency tables, see \citet{hoem87,jmh11} and
\citet{nldo81}. This is interesting: Because there were no good statistical
software for survival analysis around 1980, innovative methods like the
log-linear models were found to work outside their so-called natural
domain. When later new software appeared for direct survival analysis these
...
\subsection{Stopping and spacing}

We consider the development of a stopping behavior both by period and by
cohort. It is measured by the {\tt 'm'} of \citet{ct78}. Similarly, the
spacing behavior is measured by their {\tt 'M'}. We show the development by
period and by cohort, in both cases for each {\tt hisclass}, adjusted for
{\tt marriage age} and {\tt parish}.  
The results are only shown graphically, and the reason is that there are many three-way interactions involved, see Table~\ref{tab:sigthree}.
...
This model contains no less than 240 estimated coefficients: You do not want to see them in print, and I certainly do not want to show them. Instead, first I show the statistical significance of the three-way interactions in 
Table~\ref{tab:sigthree}. Then, in what follows, the results are graphically reported. The development over time may be driven by time periods or cohorts (or both). 


\subsubsection{By period}

From Figure~\ref{fig:aggre} is seen that there are two time periods in the nineteenth century, {\tt 1851--1880} and {\tt 1881-1900}, and five ten-year-long periods in the twentieth century. For each period, and for each {\tt hisclass}, a Poisson regression model is fitted as follows (exemplified with the period {\tt 1931-1940} and {\tt hisclass = 3}):
<<exfit,echo=TRUE>>=
fit <- glm(event ~ offset(log(N * exposure)) + V + urban + marAge,
           data = kTab, family = poisson,
           subset = (hisclass == "farmer" & period == "1931-1940"))
@
The result is shown in Table~\ref{tab:tableexfit}.

<<tableexfit, results = 'asis'>>=
xtable(round(summary(fit)$coef[, 1:2], 4), digits = 4, caption = "Parameter estimates, Poisson regression.", label = "tab:tableexfit")
@

From this we get that $\hat{M} = \exp(-0.3716) = 0.690$ and $\hat{m} = 0.499$, that is, strong signs of both spacing (small {\tt M}) and stopping (large {\tt m}). Note that the $M = \exp(Intercept)$ is evaluated for the reference category, \emph{rural} residence and \emph{marriage age} {\tt (15, 25]}.

This is then repeated for all hisclasses and all periods, and the results are shown in Figure~\ref{fig:plotmhc}, where it is clear that the {\tt elite} is introducing a \emph{stopping} behavior already in the late nineteenth century (lower panel), while the {\tt middle class} is leading in lowering the overall level, or \emph{spacing}. Farmers are slow in both respects.
...
<<plotmhc, fig.cap = "Coale-Trussell's M and m by hisclass and period.",fig.height = 9>>=
source("R/getMmhc.R")
...
## 'M':
plot(x, y1[1, -1], col = 1, type = "b", ylab = "M", ylim = c(0, 1.1),
...
<<plotmcohhc, fig.cap = "Coale-Trussell's M and m by hisclass and cohort.", fig.height = 9>>=
kohort <- kTab[!(kTab$cohort %in% c("1921-1940", "1941-1960")), ]
kohort$cohort <- factor(kohort$cohort)
...
oldpar = par(mfrow = c(2, 1))
## M:
plot(x, y1[1, ], col = 1, type = "b", ylab = "M", ylim = c(0, 1.3),
...
legend("bottomleft", legend = levels(kohort$hisclass), 
...
We repeat the analyses with \emph{TMFR} replacing $(M, m)$. 
...
<<plotmhcTMFR, fig.cap = "TMFR by hisclass and period.">>=
##source("R/getMmhc.R")
source("R/tmfrCT.R")
...
box()
for (j in 2:6){
    lines(x, y[j, -1], col = j, type = "b", pch = j, lty = j, lwd = 3)
}
...
<<plotcohortTMFR, fig.cap = "TMFR by hisclass and cohort.">>=
##source("R/getMmhc.R")
source("R/tmfrCT.R")
...
pp <- levels(kohort$cohort)
for (i in 1:5){
    res <- getMmhc(kohort[kohort$cohort == pp[i], ])
    for (j in 1:6){
        y[j, i] <- tmfrCT(res[j, 1], res[j, 2])
    }
    }
x <- c(1830, 1850, 1870, 1890, 1910)
plot(x, y[1, ], col = 1, type = "b", ylab = "TMFR", ylim = c(0, 10),
...
box()
for (j in 2:6){
    lines(x, y[j, ], col = j, type = "b", pch = j, lty = j, lwd = 3)
}
legend("bottomleft", legend = levels(kTab$hisclass), col = 1:6, pch = 1:6, lty = 1:6, cex = 0.75)
abline(h = 0)
@
...
<<cohorttmfr, fig.cap = "Number of births per woman by hisclass and cohort.">>=
source("R/kohort.R")
...
x <- c(1830, 1850, 1870, 1890, 1910)
plot(x, y[1, ], type = "b", ylim = c(0, 5), xlab = "Cohort", ylab = "Births per woman", las = 1, lwd = 3)
axis(4, las = 1)
for(i in 2:6){
    lines(x, y[i, ], col = i, lty = i, pch = i, type = "b", lwd = 3)
}
abline(h = 0)
@
...
\section{Analysis, surviving births}

We define (for now) a \emph{surviving birth} as a birth of a child that
...
With the new data set, where {\tt event} is replaced by {\tt event1} (indicating 
...
<<plotmhc1, fig.cap = "Coale-Trussell's m by hisclass and period, surviving births.">>=
source("R/getMmhc.R")
y <- matrix(0, ncol = 8, nrow = 6)
##kTab <- kTab[kTab$region == "ske", ]
...
x <- c(1865, 1890, 1905, 1915, 1925, 1935, 1945)
plot(x, y[1, -1], col = 1, type = "b", ylab = "m", ylim = c(-0.2, 1.3), 
     axes = FALSE, xlab = "Period", xlim = c(1851, 1951), lwd = 3)
axis(1, at = c(1851, x, 1950), las = 2)
axis(2, las = 1)
axis(4, las = 1)
...
<<plotmcohhc1, fig.cap = "Coale-Trussell's m by hisclass and cohort, surviving births.">>=
kohort <- kTab.s[!(kTab.s$cohort %in% c("1921-1940", "1941-1960")), ]
kohort$cohort <- factor(kohort$cohort)
y <- matrix(0, ncol = 5, nrow = 6)
##kTab <- kTab[kTab$region == "ske", ]
pp <- levels(kohort$cohort)
...
x <- c(1830, 1850, 1870, 1890, 1910)
plot(x, y[1, ], col = 1, type = "b", ylab = "m", ylim = c(-0.2, 1.3),
...
The TMFR for survivors are shown in Figure~\ref{fig:plotmhcTMFR1}. Here we can see slightly lower levels than in the case with all births, which is quite natural since the TMFR directly measures level in contrast to Coale-Trussell's {\tt m}.

<<plotmhcTMFR1, fig.cap = "TMFR by hisclass and period, surviving births.">>=
##source("R/getMmhc.R")
y <- matrix(0, ncol = 8, nrow = 6)
##kTab <- kTab[kTab$region == "ske", ]
pp <- levels(kTab.s$period)
...
<<cohorttmfr1, fig.cap = "Number of surviving births per woman by hisclass and cohort.">>=
source("R/kohort.R")
kids.s <- kids
...
x <- c(1830, 1850, 1870, 1890, 1910)
plot(x, y[1, ], type = "b", ylim = c(0, 5), xlab = "Cohort", ylab = "TMFR", las = 1, lwd = 3)
axis(4, las = 1)
for(i in 2:6){
    lines(x, y[i, ], col = i, lty = i, pch = i, type = "b", lwd = 3)
}
...
The question of whether it is spacing or stopping that dominates the behavioral explanation of the fertility decline over time is not definitively settled, but the stopping measure, Coale-Trussell's "little {\tt m}" shows a more dramatic change over time than the corresponding spacing measure "big {\tt M}". This is especially true in the cohort setting.
...
{\bf R} environment for statistical analysis \citep{cran},  mainly with the package {\bf eha} \citep{eha,ehar12}.
